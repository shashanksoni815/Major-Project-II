...
router.post('/', async (req, res) => {
  try {
    ...
    const staff1 = await Staff.create({ name: 'Alice', role: 'Lab Assistant', email: 'alice@example.com', phone: '1234567890', passwordHash: await bcrypt.hash('password', 8), labIds: [lab1._id] })
    const staff2 = await Staff.create({ name: 'Bob', role: 'Faculty', email: 'bob@example.com', phone: '0987654321', passwordHash: await bcrypt.hash('password', 8), labIds: [lab2._id] })
